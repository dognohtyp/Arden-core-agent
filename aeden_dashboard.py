...
# Setup paths dynamically relative to the script location
base_path = os.path.dirname(__file__)
seed_path = os.path.join(base_path, "Seeds", "gpt_full_clone_core.txt")
memory_path = os.path.join(base_path, "Memory", "brain.mem.txt")

os.makedirs(os.path.dirname(memory_path), exist_ok=True)
os.makedirs(os.path.dirname(seed_path), exist_ok=True)

# Dynamic local fallback path for the model file
...
# Initialize Local LLM Infrastructure
model = GPT4All(
...
def load_mind():
    ...
    if os.path.exists(memory_path):
        with open(memory_path, "r", encoding="utf-8") as f:
            # Sliding memory window tracking to protect token limits
...
# Desktop Interface Setup
root = tk.Tk()
root.title("Aeden Chat Interface (Memory + Voice)")
